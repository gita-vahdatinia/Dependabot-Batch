...
if [[ $ALL_OPEN_PRS ]]; then
  echo "Grabbing all of the currently open dependabot prs"
  git fetch --all
  branches=()
  eval "$(  git branch --all --format='branches+=(%(refname:short))' )"
  echo ${branches[@]}
  for branch in "${branches[@]}"; do
      if [[ $branch = origin/dependabot* ]]; then
          branch_name=${branch/\origin\/} 
          echo "Checking out ${branch_name}"
          LIST_OF_BRANCHES+="$branch_name "
          echo "${LIST_OF_BRANCHES}"
      fi
  done
fi
LIST_OF_BRANCHES=$(echo -e "${LIST_OF_BRANCHES}" | xargs)
echo ${LIST_OF_BRANCHES}
PATCHED=()
...
Generated by: ${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/actions/runs/${GITHUB_RUN_ID} ${TICK}  ${PATCHED[*]}${TICK}
...
echo "--------------------------------------------------------------------------------"
echo "https://github.com/${GITHUB_REPOSITORY}/pull/${PULL_NUMBER}"
